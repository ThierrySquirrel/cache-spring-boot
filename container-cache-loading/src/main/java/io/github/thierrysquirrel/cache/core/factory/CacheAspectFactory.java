/**
 * Copyright 2026/6/2 ThierrySquirrel
 * <p>
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p>
 * http://www.apache.org/licenses/LICENSE-2.0
 * <p>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 **/
package io.github.thierrysquirrel.cache.core.factory;

import io.github.thierrysquirrel.cache.core.exception.CacheException;
import io.github.thierrysquirrel.cache.core.template.CaffeineTemplate;

import java.util.Objects;
import java.util.concurrent.Callable;
import java.util.logging.Level;
import java.util.logging.Logger;

/**
 * ClassName: CacheAspectFactory
 * Description:
 * date: 2026/6/2
 *
 * @author ThierrySquirrel
 * @since JDK 25
 */
public class CacheAspectFactory {

    private static final Logger logger = Logger.getLogger(CacheAspectFactory.class.getName());

    private CacheAspectFactory() {
    }

    public static Object loadCaffeineCache(Callable<?> callable, CaffeineTemplate caffeineTemplate, String key) throws CacheException {
        Object cache = caffeineTemplate.get(key);
        if (Objects.isNull(cache)) {
            return releaseTask(callable, caffeineTemplate, key);
        }
        return cache;
    }


    public static Object clearCache(Callable<?> callable, CaffeineTemplate caffeineTemplate, String key) throws CacheException {
        Object proceed;
        try {
            proceed = callable.call();
            caffeineTemplate.invalidate(key);
        } catch (Throwable throwable) {
            String logMsg = "Method execution failed!";
            logger.log(Level.WARNING, logMsg, throwable);
            throw new CacheException(logMsg, throwable);
        }
        return proceed;
    }

    private static Object releaseTask(Callable<?> callable, CaffeineTemplate caffeineTemplate, String key) throws CacheException {
        Object proceed;
        try {
            proceed = callable.call();
            caffeineTemplate.put(key, proceed);
        } catch (Throwable throwable) {
            String logMsg = "Method execution failed!";
            logger.log(Level.WARNING, logMsg, throwable);
            throw new CacheException(logMsg, throwable);
        }

        return proceed;
    }
}
